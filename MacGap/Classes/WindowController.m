...
//    [self.webView setMainFrameURL:[self.url absoluteString]];
    [self.webView.mainFrame loadHTMLString:UNCOMPRESSED_HTML
                                   baseURL:[[NSURL alloc]initFileURLWithPath:[[NSBundle mainBundle] resourcePath]]];
...
//    win.titleBarHeight = 0.0;
    self.window = win;
    
//    Move *move = [[Move alloc] initWithFrame:NSMakeRect(0, self.window.frame.size.height - 50, self.window.frame.size.width, 50)];
//    [self.window.contentView addSubview:move];
    
//    NSButton *closeButton = [NSWindow standardWindowButton:NSWindowCloseButton forStyleMask:NSBorderlessWindowMask];
//    [closeButton setFrame:NSMakeRect(8, self.window.frame.size.height - 9, 13, 13)];
//    
...
- (void) awakeFromNib
{
    ...
    NSDictionary *plugins = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:@"App", @"Sound", @"Dialog", @"Notify", nil]
                                                        forKeys:[NSArray arrayWithObjects:@"App", @"Sound", @"Dialog", @"Notify", nil]];
//    NSDictionary *plugins = [NSDictionary dictionaryWithObjectsAndKeys:@"Sound", @"Sound", "Dialog", nil]
...
- (void) setWindowParams
{
    ...
    if([params objectForKey:@"alpha"]) {
        NSColor *backgroundColor = [NSColor colorWithDeviceRed:255.0 green:255.0 blue:255.0 alpha:[[params objectForKey:@"alpha"] doubleValue]];
